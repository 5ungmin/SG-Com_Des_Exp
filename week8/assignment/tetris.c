...
//    nextBlock[2]=rand()%7;
    ...
    for(j=0;j<HEIGHT;j++)
        for(i=0;i<WIDTH;i++)
            recRoot->recField[j][i]=field[j][i];
    recRoot->recommendR = 0;
    recRoot->recommendY = 0;
    recRoot->recommendX = 0;
...
void DrawBlockWithFeatures(int y, int x, int blockID, int blockRotate) {
    ...
    DrawRecommend(recRoot->recommendY, recRoot->recommendX, blockID, recRoot->recommendR); //week8
}
//************************************
    ...
    for (rotate = 0; rotate < NUM_OF_ROTATE; rotate++) {
        ...
        for (x = -3; x < WIDTH+3; x++) {
            ...
            if (CheckToMove(root->recField, nextBlock[root->level], rotate, 0, x)) {
                ...
                score += AddBlockToField(child[n]->recField, nextBlock[root->level], rotate, y, x);
        ...
        for (x = Xmin; x <= Xmax; x++) {                            //modified
            ...
            if (CheckToMove(root->recField, nextBlock[root->level], rotate, 0, x)) {
                ...
                //pruning
                    ...
                    if (score >= VISIBLE_BLOCKS * VISIBLE_BLOCKS * VISIBLE_BLOCKS * 10) { //pruning!!
//                        fprintf(testfp, "%d\n", score);
                        score += modified_recommend(child[n]); //recursive call
                    }
                }
...
#ifdef TESTTIME
    if (score > 50000) gameOver = 1;
#endif
}

int RecProcessCommand(int command){
    int ret=1;
    switch(command){
    case QUIT:
        ret = QUIT;
